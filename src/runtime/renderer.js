...
function patch(_vnode, vnode, container) {
  if (!_vnode) {
  ...
  const { shapeFlag } = vnode;
  if (shapeFlag & ShapeFlags.ELEMENT) {
    patchElement(_vnode, vnode, container);
  } else if (shapeFlag & ShapeFlags.TEXT) {
    patchText(_vnode, vnode, container);
  } else if (shapeFlag & ShapeFlags.FRAGMENT) {
    patchFragment(_vnode, vnode, container);
  } else if (shapeFlag & ShapeFlags.COMPONENT) {
    patchComponent(_vnode, vnode, container);
  }
}
...
function mountChildren(vnode, container) {
  ...
  if (shapeFlag & ShapeFlags.TEXT_CHILDREN) {
    mountText(vnode, container); // 在mountText中取出children
...
function unmountChildren(vnode) {
  /* 在生成的vnode中（h函数）添加el属性 */
  const { shapeFlag, el } = vnode;
  if (shapeFlag & ShapeFlags.TEXT_CHILDREN || shapeFlag & ShapeFlags.ELEMENT) {
...
function patchFragment(_vnode, vnode) {}

function patchComponent(_vnode, vnode) {}

function patchChildren(_vnode, vnode, container) {
  /* 更新children函数 */
  ...
  if (shapeFlag & ShapeFlags.TEXT_CHILDREN) {
    ...
    if (_shapeFlag & ShapeFlags.TEXT_CHILDREN) {
      container.textContent =
        vnode.textContent; /* 虚拟DOM应该保持在children中，后续检查以辨别是否与父元素的虚拟dom保持一致 */
    ...
    if (_shapeFlag & ShapeFlags.TEXT_CHILDREN) {
      ...
      // patchArrayChildren(_children, children, container)
    } else if (_vnode === null) {
      mountChildren(vnode, container);
    }
  } else if (vnode === null) {
